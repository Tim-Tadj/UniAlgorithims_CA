    ...
    if(len(array[0])//2 >= divideAt): #if the folding point is less/equal to halfway along the punch car run this
        ...
        for i in range(rows):
            for j in range(divideAt): #concatinate the result after the excess has been done
                if array[i][j] == array[i][divideAt*2-j-1]: #compare counting up the string while counting down from the other side of the fold point
                    result[i] += array[i][j]
                else: #if not matched default to no hole
                    result[i]+= 'x' 
        ...
        for i in range(rows):
            ...
            for j in range(start, divideAt):
                if array[i][j] == array[i][len(array[0])-1-y]: #compare counting up the string while counting down from the other side of the fold point
                    result[i]+= array[i][j]
                else: #if not matched default to no hole
                    result[i]+= 'x'
                y+=1
